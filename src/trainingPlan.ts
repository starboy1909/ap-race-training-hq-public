...
export function buildPlan(choice: MarathonChoice='none'): Week[] {
 ...
 for(let n=0;n<39;n++){
  ...
  for(let dow=0;dow<7;dow++){
   ...
   const day:Day={id:`S${19+n}-${dow}`,iso,date:date.toLocaleDateString('en-GB',{day:'2-digit',month:'short',timeZone:'UTC'}).toUpperCase(),dow:['MON','TUE','WED','THU','FRI','SAT','SUN'][dow],title:'',type:'EASY',duration:'',rpe:'',...spec,travel};day.secondary=second(day);days.push(day);
  }
  output.push({id:`S${19+n}`,label:`W${19+n}`,dates:`${days[0].date} – ${days[6].date} ${start.slice(0,4)}`,phase:days.some(d=>d.race)?'RACE':days.some(d=>d.travel)?'ADAPT':deload?'DELOAD':'BUILD',volume:'Follow daily prescriptions · optional work adds load',focus:n<4?'Build repeatable sessions before raising volume. Two strength days, one running-quality day, one controlled HYROX day and one easy long run.':'Provisional continuation: review every week against actual completions and recovery. Future paces do not become faster automatically.',gate:'Proceed when walking/stairs are comfortable and energy is normal. If pain rises, alters gait, or is worse the next morning, stop impact and use easy non-impact work only if comfortable. Persistent focal shin pain, swelling or rest pain needs assessment.',days});
 }
 return output;
}
