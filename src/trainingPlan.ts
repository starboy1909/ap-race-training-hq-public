...
export const scheduleChangePolicy = [
 ['Plan of record', 'The published primary workout stays scheduled unless Amar explicitly approves a change or reports symptoms that require modification.'],
 ['Device-data rule', 'An isolated Garmin metric or a harder-than-planned session creates a coaching flag only. Pace, heart rate, duration, sleep, HRV or Body Battery alone cannot replace a future workout.'],
 ['Safety exception', 'A verified acute injury, illness or severe recovery signal may place the session on HOLD for same-day reassessment. It must not silently substitute a different workout.'],
 ['Start-line check', 'On the day: proceed when normal walking and stairs are comfortable, there is no illness, and the warm-up feels normal. Stop or downgrade during the session if pain, breathing or mechanics become abnormal.'],
];
...
export function buildPlan(choice: MarathonChoice='none'): Week[] {
 ...
 for(let n=0;n<39;n++){
  ...
  for(let dow=0;dow<7;dow++){
   ...
   if(iso==='2026-09-26')spec={title:'Completed treadmill run + strength',type:'RUN',duration:'60:01 run + 31:40 strength · completed',rpe:'Completed',blocks:[block('VERIFIED COMPLETION',['10.48 km treadmill run in 60:01 (approximately 5:44/km), followed by 31:40 of strength work.','This substantially exceeded the planned travel-day recovery load. It is a useful completion, but not enough repeatable evidence to raise the pace guide.','No catch-up work. Keep Sunday as recovery only and resume the next build week only if walking, stairs, energy and warm-up mechanics are normal.'])],note:'The combined session is a material load deviation during reduced availability. Prioritise food, fluids and sleep; do not add a second workout.'};
   if(iso==='2026-09-27')spec={...rest(),title:'Recovery after completed run + strength',type:'RECOVERY',note:'No catch-up load. Optional relaxed walking and mobility only; resume Monday only if recovery markers and normal movement are reassuring.'};
